 ...
 * the original body through a `pull`-driven ReadableStream that counts
 * as it goes. Both `arrayBuffer()` and `text()` consume `.body` so the
 * cap applies to every read path Python code uses, including
 * `pyodide.http.pyfetch`'s own `bytes()`/`.string()` helpers.
 *
 * Why `pull` and not `start`: an earlier draft put the entire read loop
 * inside `start()`. That kept the wrapped stream in the "starting" state
 * until the source was fully drained, and any `await reader.cancel()`
 * inside that loop deadlocked against the consumer that was waiting for
 * the start promise to resolve. `pull` is the canonical pattern — the
 * runtime invokes it each time the consumer needs more data, so
 * backpressure works and `cancel` becomes fire-and-forget.
 */
    ...
    async pull(controller) {
      try {
        ...
        if (received > maxBytes) {
          const err = new ResponseTooLargeError(received, maxBytes);
