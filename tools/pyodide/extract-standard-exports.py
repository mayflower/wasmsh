...
all function export names to stdout, one per line, each ensured to
start with '_' (the Emscripten EXPORTED_FUNCTIONS convention).
...
def read_leb128(data, pos):
    ...
    while True:
        if pos >= len(data):
            raise ValueError(f"Truncated LEB128 at offset {pos}")
        byte = data[pos]
        pos += 1
...
def extract_function_exports(wasm_path):
    ...
    try:
            ...
            if section_id == 7:  # Export section
                ...
                for _ in range(count):
                    ...
                    if pos + name_len > len(data):
                        raise ValueError(
                            f"Truncated export name at offset {pos}"
                        )
        ...
        print(
            f"error: failed to parse {wasm_path}: {e}",
            file=sys.stderr,
        )
        sys.exit(1)
